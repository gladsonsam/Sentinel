...
export function ConnectionStatus({ connected, lastSeen }: ConnectionStatusProps) {
  ...
  const getOfflineText = () => {
    ...
    if (diffDay > 0) return `Offline (${diffDay}d ago)`;
    if (diffHour > 0) return `Offline (${diffHour}h ago)`;
    if (diffMin > 0) return `Offline (${diffMin}m ago)`;
    return `Offline (${diffSec}s ago)`;
  };
