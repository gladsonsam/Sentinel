  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto">
        {list.length === 0 ? (
          <p className="px-3.5 py-3 text-xs text-muted italic">
            No agents online
          </p>
        ) : (
          list.map((agent) => (
            ...
            >
              <span className="text-[13px] font-medium flex items-center gap-1.5 truncate">
                <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 animate-pulse"
                    style={{ background: agent.online ? 'var(--color-ok)' : 'var(--color-muted, #888)' }} />
                {agent.name}
              </span>
              <span className="text-[11px] text-muted tabular-nums">
                {agent.online
                  ? relativeTime(agent.connected_at)
                  : `Last seen ${relativeTime(agent.last_disconnected_at ?? agent.last_seen)}`
                }
              </span>
            </button>
          ))
